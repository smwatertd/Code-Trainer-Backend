...
@pytest.mark.asyncio(loop_scope="session")
async def test_curriculum_links__migration_seed_on_task4(
    teacher_client: tuple[AsyncClient, E2EAuthUser],
) -> None:
    client, auth_user = teacher_client
    headers = await auth_headers(client, auth_user)

    response = await client.get("/api/curriculum/tasks/4/links", headers=headers)

    assert response.status_code == 200
    body = response.json()
    assert body["has_curriculum_link"] is True
    assert body["primary_link"]["technical_concept_id"] == "for_loop"
    assert body["primary_link"]["exercise_pattern_id"] == "tr_pattern_translation"
    assert body["primary_link"]["learning_concept_id"] == "loops"
...
async def test_curriculum_links__primary_swap_clears_old_primary(
    ...
    before = await client.get("/api/curriculum/tasks/4/links", headers=headers)
    assert before.status_code == 200
    old_primary_id = before.json()["primary_link"]["id"]
    ...
    metadata = await client.get("/api/curriculum/tasks/4/links", headers=headers)
    assert metadata.status_code == 200
    body = metadata.json()
    assert body["primary_link"]["id"] == new_primary_id
    assert body["primary_link"]["technical_concept_id"] == "while_loop"
