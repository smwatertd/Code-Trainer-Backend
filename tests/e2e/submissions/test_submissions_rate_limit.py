...
async def test_submissions__concurrent_queued_jobs_hit_rate_limit(
    ...
    first = await client.post("/api/submissions", headers=headers, json=TASK1_HELLO_PYTHON)
    second = await client.post("/api/submissions", headers=headers, json=TASK2_BLOCK_REORDER_OK)
