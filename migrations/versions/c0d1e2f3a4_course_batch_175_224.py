...
_COURSE_BATCH_175_224_PREV_SIZE = 174

revision: str = "c0d1e2f3a4_course_batch_175_224"
down_revision: Union[str, Sequence[str], None] = "b9c0d1e2f3_course_batch_125_174"
...
def upgrade() -> None:
    ...
    for task in TASK_CATALOG_SEED:
        if task["id"] <= _COURSE_BATCH_175_224_PREV_SIZE:
            continue
        row = dict(task)
    ...
    existing = connection.execute(
        ...
        {"min_id": _COURSE_BATCH_175_224_PREV_SIZE},
    ).first()
    if existing:
        return
...
def downgrade() -> None:
    ...
    connection.execute(
        sa.text(f'DELETE FROM "{schema}"."tasks" WHERE id > {_COURSE_BATCH_175_224_PREV_SIZE}')
    )
    connection.execute(
        sa.text(
            f'ALTER SEQUENCE "{schema}"."tasks_id_seq" RESTART WITH {_COURSE_BATCH_175_224_PREV_SIZE + 1}'
        )
    )
