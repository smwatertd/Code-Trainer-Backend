...
_COURSE_BATCH_125_174_PREV_SIZE = 124

revision: str = "b9c0d1e2f3_course_batch_125_174"
down_revision: Union[str, Sequence[str], None] = "a8b9c0d1e2_course_batch_75_124"
...
def upgrade() -> None:
    ...
    for task in TASK_CATALOG_SEED:
        if task["id"] <= _COURSE_BATCH_125_174_PREV_SIZE:
            continue
        row = dict(task)
    ...
    existing = connection.execute(
        ...
        {"min_id": _COURSE_BATCH_125_174_PREV_SIZE},
    ).first()
    if existing:
        return
...
def downgrade() -> None:
    ...
    connection.execute(
        sa.text(f'DELETE FROM "{schema}"."tasks" WHERE id > {_COURSE_BATCH_125_174_PREV_SIZE}')
    )
    connection.execute(
        sa.text(
            f'ALTER SEQUENCE "{schema}"."tasks_id_seq" RESTART WITH {_COURSE_BATCH_125_174_PREV_SIZE + 1}'
        )
    )
